...
func run(ctx context.Context) error {
	...
	// request_tag: app=tag-sandbox,tx=ro1,h=1bb5077c
	if err = tx.QueryWithHash(ctx, spanner.NewStatement("SELECT 1")).Do(nop); err != nil {
		return err
	}

	// request_tag: app=tag-sandbox,tx=ro1,h=9087fd8b
	...
	// request_tag: app=tag-sandbox,tx=ro1,query=FindAllSingers
	...
	grp.Go(func() error {
		// transaction_tag: app=tag-sandbox,tx=rw1,
		_, err = client.ReadWriteTransaction(ctx, []spannerplus.Tag{{"tx", "rw1"}},
			func(ctx context.Context, transaction *spannerplus.ReadWriteTransactionWrapper) error {
				// request_tag: app=tag-sandbox,tx=rw1,h=f0e76052
	...
	grp.Go(func() error {
		// transaction_tag: app=tag-sandbox,tx=rw2
		_, err = client.ReadWriteTransactionTagString(ctx, "tx=rw2",
			func(ctx context.Context, transaction *spannerplus.ReadWriteTransactionWrapper) error {
				// request_tag: app=tag-sandbox,tx=rw2,h=f0e76052
	...
	grp.Go(func() error {
		// transaction_tag: app=tag-sandbox,tx=rw3
		_, err = client.ReadWriteTransaction(ctx, []spannerplus.Tag{{"tx", "rw3"}},
			func(ctx context.Context, transaction *spannerplus.ReadWriteTransactionWrapper) error {
				// request_tag: app=tag-sandbox,tx=rw3,h=69daf312
				err := transaction.QueryWithHash(ctx,
